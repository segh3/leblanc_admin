document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        ...
        function validarLoginEmail() {
            ...
            const rx = /@(duoc\.cl|profesor\.duoc\.cl|gmail\.com)$/;
            if (!v || !rx.test(v)) {
                setError(loginEmail, 'Correo no válido. Debe ser @duoc.cl, @profesor.duoc.cl o @gmail.com.');
                return false;
            }
            setOk(loginEmail);
            return true;
        ...
        function validarLoginPassword() {
            ...
            if (v.length < 4 || v.length > 10) {
                setError(loginPassword, 'La contraseña debe tener entre 4 y 10 caracteres.');
                return false;
            }
            setOk(loginPassword);
            return true;
        ...
        loginForm.addEventListener('submit', (event) => {
            event.preventDefault();
            const ok = validarLoginEmail() & validarLoginPassword();
            if (ok) {
